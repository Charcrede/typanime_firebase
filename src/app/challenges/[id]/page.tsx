...
import { useState, FormEvent, useEffect, SetStateAction, useRef, cache } from 'react';
...
const Citations = ({ params: { id } }: { params: { id: string } }) => {
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}/api/challenges/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('k')}` } }).then((resp) => {
            console.log(resp.data);
            setStatList(resp.data.statList)
            setCit(resp.data.citation ? resp.data.citation : resp.data.synopsis)
            setTimeout(() => {
    ...
    const handleChange = (e: {
        ...
        if (spans) {
            ...
            if (l == text.length) {
                ...
                if (user) {
                    axios.post(`${apiUrl}/api/stats/create`, { accuracy: accuracy, speed: speed, citationId: cit?.id, username: user.u.username, challengeId: id }, { headers: { Authorization: `Bearer ${user.token}` } }).then((resp) => {
                        axios.get(`${apiUrl}/api/challenges/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('k')}` } }).then((resp) => {
                            setStatList(resp.data.statList)
                        })
                    })
    ...
    const timing = (num: number) => {
        ...
        let intervalId = setInterval(() => {
            ...
            if (entryRef.current.length > 4) {
                ...
                if (s < 0 || Math.floor(((activeTimer - errorCount) / activeTimer) * 100) < 50) {
                    console.log("Condition remplie, speed = 0");
                    setSpeed(0);
                } else {
                    console.log("Condition non remplie, speed =", s);
                    setSpeed(s);
                }
    ...
    return (
        < >
            <Nav></Nav>
            <div className='lg:px-8 xs:z-0 xs:px-4 flex gap-8'>
                <div className='w-3/4'>
                    <div className='lg:text-xl font-semibold xs:text-lg'>
                        <span className='bg-white text-primary mr-2 p-1 border-2 border-white xs:hidden lg:inline'>{cit?.persoName ? "Citation" : "Synopsis"} #{cit?.id}</span>
                        <span className='border-2 border-white text-white p-1'>{cit && "persoName" in cit ? cit?.persoName : cit?.anime}</span>
                    ...
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="bg-gray-200 text-gray-700">
                                ...
                                <th className="p-2 text-left">Vitesse</th>
                                <th className="p-2 text-left">Précision</th>
                            </tr>
                        </thead>
                        <tbody>
                            {statList.map((el, i) => (
                                <tr className="border-b" key={i}>
                                    ...
                                    <td className="p-2">{el.user.username}</td>
                                    <td className="p-2">{el.speed} MPM</td>
                                    <td className="p-2">{el.accuracy}%</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
